...
RUN . $SPACK_ROOT/share/spack/setup-env.sh && spack spec octopus +netcdf+arpack+cgal+python+likwid+libyaml+nlopt+check_short
...
RUN . $SPACK_ROOT/share/spack/setup-env.sh && spack install octopus +netcdf+parmetis+arpack+cgal+python+likwid+libyaml+nlopt+check_short
